class StreetlightMap {
    ...
    setupMap() {
        ...
        Object.entries(geoJsonFiles).forEach(([regionCode, fileName]) => {
            ...
            `;
                                    ...
                                    if (!isMobile && !layer.isClicked) {
                                        ...
                                        if (
                                            ...
                                            layer.hoverTooltip = L.tooltip({
                                                ...
                                                direction: "center",
                                                className:
                                                    "province-name-tooltip hover-tooltip",
                                                offset: [0, 0],
                                            })
                                                .setContent(
                                                    feature.properties.name
                                ...
                                click: (e) => {
                                    ...
                                    if (layer.isClicked) {
                                        ...
                                        if (this.activeGeoJsonLayer) {
                                            ...
                                            if (
                                                this.activeGeoJsonLayer
                                                    .clickTooltip
                                            ) {
                                                this.activeGeoJsonLayer.clickTooltip.remove();
                                                this.activeGeoJsonLayer.clickTooltip =
                                                    null;
                                            }
                                            if (
                                                this.activeGeoJsonLayer
                                                ...
                                                this.activeGeoJsonLayer.hoverTooltip =
                                                    null;
                                            }
                                            if (
                                                this.activeGeoJsonLayer
                                                ...
                                                this.activeGeoJsonLayer.nameTooltip =
                                                    null;
                                            }
                                        }
                                        ...
                                        if (
                                            ...
                                            layer.clickTooltip = L.tooltip({
                                                ...
                                                direction: "center",
                                                className:
                                                    "province-name-tooltip click-tooltip",
                                                offset: [0, 0],
                                            })
                                                .setContent(
                                                    feature.properties.name
                ...
                .catch((error) => {
                    console.error(
                        `Error loading GeoJSON for ${regionCode}:`,
                        error
                    );
                });
        });
    ...
    async initializeMap() {
        ...
        }).setView([9.1600593, 125.872463], parseFloat(8.8));
    ...
    getBarangayCode(socid) {
        ...
        if (parts.length !== 2) {
            console.log(
                `Invalid SOCID format: ${socid} (expected format: XXX-YYYY)`
            );
            return null;
        }
        ...
        try {
            ...
            if (data.status === "success" && Array.isArray(data.data)) {
                return data.data.filter((reading) => {
                    const parts = reading.socid?.split("-");
                    return (
                        parts && parts[1] && parts[1].startsWith(barangayCode)
                    );
                }).length;
            }
            return 0;
    ...
    getBarangayCoordinates(barangayCode, province) {
        ...
        for (const municipalityName in this.coordinates[province]
            .municipalities) {
            const municipality =
    ...
    async showMunicipalityMarkers(province) {
        ...
        try {
            // Get province data from coordinates
            ...
            const normalizeProvinceName = (name) => {
                // Convert "AGUSAN DEL NORTE" to "Agusan Del Norte"
                return name
                    .split(" ")
                    .map(
                        (word) =>
            ...
            // Get province code from CONFIG
            const provinceCode =
                CONFIG.PROVINCES.CODES[normalizeProvinceName(province)];
            if (!provinceCode) {
                console.error("Province code not found for:", province);
                console.log(
                    "Available provinces:",
                    Object.keys(CONFIG.PROVINCES.CODES)
                );
                return;
            }
            ...
            for (const municipalityName in provinceData.municipalities) {
                const municipalityData =
                    provinceData.municipalities[municipalityName];
                const municipalityCode =
                    CONFIG.MUNICIPALITIES.CODES[municipalityName];
                ...
                try {
                    // Fetch municipality statistics from API
                    const response = await fetch(
                        `${CONFIG.API.BASE_URL}${CONFIG.API.ENDPOINTS.MUNICIPALITY_COUNT}/${provinceCode}/${municipalityCode}`
                    );
                    ...
                    if (statsData.status !== "success") {
                        console.error(
                            `Failed to fetch stats for ${municipalityName}`
                        );
                        continue;
                    }
                    ...
                    const markerColor =
                        stats.inactive > 0 ? "text-danger" : "text-primary";
                    ...
                    const marker = L.marker(
                        [municipalityData.lat, municipalityData.long],
                        {
                            icon: L.divIcon({
                                className: "custom-marker",
                                html: `<i class="fas fa-map-marker-alt ${markerColor} fa-2x"></i>`,
                                iconSize: [30, 30],
                                iconAnchor: [15, 30],
                            }),
                        }
                    ...
                    marker.on("popupopen", (e) => {
                        const popup = e.popup;
                        const button =
                            popup._contentNode.querySelector(".view-details");
                        if (button) {
                            button.addEventListener("click", () => {
                                this.showBarangayMarkers(
                                    province,
                                    municipalityName
                                );
                                this.map.flyTo(
                    ...
                    console.error(
                        `Error fetching data for ${municipalityName}:`,
                        error
                    );
                }
            }
    ...
    handleZoom() {
        try {
            ...
            if (zoom < 9 || zoom < this.zoomLevels.city) {
                ...
                Object.entries(geoJsonFiles).forEach(
                    ([regionCode, fileName]) => {
                        fetch(`rsc/geojson/${fileName}`)
                            .then((response) => {
                                if (!response.ok) {
                            ...
                            .then((data) => {
                                // Add custom CSS styles
                                if (
                                    !document.getElementById("geojson-styles")
                                ) {
                                    const style =
                        ...
                        `;
                                ...
                                // Create GeoJSON layer
                                this.geoJsonLayers[regionCode] = L.geoJSON(
                                    data,
                                    {
                                        style: (feature) => ({
                                            color: "transparent",
    ...
    async createProvincePopup(province) {
        ...
        try {
            const provinceCode =
                CONFIG.PROVINCES.CODES[province.name] || province.code;
            const response = await fetch(
                `${CONFIG.API.BASE_URL}${CONFIG.API.ENDPOINTS.PROVINCE_COUNT}/${provinceCode}`
            );
            ...
            console.error("Error creating province popup:", error);
            container.innerHTML = `
                <div class="p-2">
        ...
        try {
            ...
            const barangayReadings = data.data.filter((reading) => {
                ...
                return (
                    parts &&
                    parts[1] &&
                    parts[1].startsWith(barangay.barangayCode)
                );
            });
            ...
            const uniqueStreetlights = Object.values(latestReadings).sort(
                (a, b) => a.socid.localeCompare(b.socid)
            );

            // Create content for the details popup
            const detailsContainer = L.DomUtil.create(
                "div",
                "streetlight-details"
            );
                    ...
                    button.addEventListener("click", async () => {
                        ...
                        try {
                            ...
                            if (data.status === "success") {
                                ...
                                document.getElementById("batv").textContent =
                                    latestReading.batv;
                                document.getElementById("batc").textContent =
                                    latestReading.batc;
                                ...
                                document.getElementById("bulbv").textContent =
                                    latestReading.bulbv;
                                document.getElementById("curv").textContent =
                                    latestReading.curv;
                                ...
                                document.getElementById(
                                    "last-update"
                                ).textContent = lastUpdate;

                                // Update status badge
                                const isActive =
                                    parseFloat(latestReading.batsoc) > 20.0;
                                const statusBadge =
                                ...
                                alert(
                                    "Error loading streetlight details: " +
                                        data.message
                                );
                            }
                        } catch (error) {
            ...
            if (data.status === "success") {
                ...
                data.data.forEach((reading) => {
                    ...
                    if (
                        !latestReadings[reading.socid] ||
                        readingDate >
                            new Date(latestReadings[reading.socid].date)
                    ) {
                        latestReadings[reading.socid] = reading;
                    }
                });
                ...
                const activeCount = Object.values(latestReadings).filter(
                    (light) => {
                        ...
                        if (isDaytime) {
                            ...
                            const isActive =
                                bulbVoltage > 10.0 && batteryCurrent < -0.1;
        ...
        setTimeout(() => {
            ...
            if (viewButton) {
                L.DomEvent.on(viewButton, "click", (e) => {
                    ...
                    const barangayCode =
                        this.coordinates[province].municipalities[municipality]
                            .barangays[barangayName].barangay_code;
    ...
    async loadStreetlightData(socid) {
        try {
            ...
            let updateIndicator = document.getElementById(
                "popup-update-indicator"
            );
            if (!updateIndicator) {
            ...
            const updateIndicator = document.getElementById(
                "popup-update-indicator"
            );
            if (updateIndicator) {
                updateIndicator.innerHTML =
    ...
    setupAutoUpdate(socid) {
        ...
        this.popupUpdateInterval = setInterval(async () => {
            ...
            try {
                ...
                const result = await StreetlightQueries.getStreetlightDetails(
                    socid
                );
    ...
    updateStreetlightDetails(readings) {
        ...
        if (latestReading.socid && latestReading.socid.includes("-")) {
            const [municipalityCode, fullBarangayId] =
                latestReading.socid.split("-");
            ...
            outerLoop: for (const province in this.coordinates) {
                const municipalities =
                    this.coordinates[province].municipalities;
                ...
                for (const municipality in municipalities) {
                    ...
                    if (
                        ...
                        for (const barangay in barangays) {
                            ...
                            const currentBarangayCode =
                                barangays[barangay].barangay_code;
                            if (
